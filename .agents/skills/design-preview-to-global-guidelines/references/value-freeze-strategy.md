...
2. Normalize background and surface separation so cards, panels, and overlays have predictable depth.
3. Establish text hierarchy with readability first and brand temperature second.
4. Normalize border strength so separators and field outlines are consistent.
5. Freeze status roles as semantic values, not decorative accents.

## Dark Theme Strategy

1. Start from semantic parity with light mode, not visual inversion.
2. Preserve hierarchy by controlling contrast bands across background, surface, and elevated surface.
