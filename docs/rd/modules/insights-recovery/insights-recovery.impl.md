...
- 路由入口：
  - 主导航洞察页、今日页恢复卡和日历历史都可进入当前周期洞察。
  - 周报详情、恢复计划详情和历史报告属于模块内部子路径，不重新拥有首页聚合状态。
- 页面状态所有权：
  - 洞察首页拥有周摘要、稳定度解释和原因分布状态。
  - 恢复计划详情拥有高优先建议、已读状态和高级承接状态。
  - 历史报告拥有锁定说明与切换周期状态。
- 返回行为：
  - 从高级详情或付费承接返回时，必须恢复当前周摘要上下文和阅读位置。
  - 从洞察跳转到订阅或恢复计划后返回时，不得丢失当前已读解释状态。

## 10. 设计源消费与实现边界

- 实现必须直接消费 [insights-recovery.ui-ux.md](D:/Projects/Flutter/rhythm/docs/rd/modules/insights-recovery/insights-recovery.ui-ux.md) 中定义的 `weekly_insight_hero_card`、`stability_explainer_panel`、`cause_distribution_card`、`recovery_plan_card` 组件边界。
- 洞察首页必须遵守“摘要 -> 解释 -> 原因 -> 恢复 -> 高级入口”的固定阅读顺序，不能在实现中把付费卡提前或把图表升格成主焦点。
- 周报聚合、稳定度算法和恢复建议推导只允许在应用层与数据层完成；显示层只消费已解释的洞察 view state。
