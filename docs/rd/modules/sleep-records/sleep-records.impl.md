...
- 路由入口：
  - 今日页、日历单日详情与我的页数据入口都可进入记录中心。
  - 补录页、修正页和原因标签弹层属于本模块内部子路径，不外泄业务状态所有权。
- 页面状态所有权：
  - 记录中心页拥有结果摘要、来源可信度与当前主动作状态。
  - 补录 / 修正页拥有表单草稿、保存中、覆盖确认和失败恢复状态。
  - 标签选择弹层拥有轻量原因补充与自定义输入状态。
- 返回行为：
  - 补录或修正成功后必须返回来源页并刷新当日摘要。
  - 删除或覆盖原记录前必须经过确认层，并在返回后保留“已修正”痕迹。

## 10. 设计源消费与实现边界

- 实现必须直接消费 [sleep-records.ui-ux.md](D:/Projects/Flutter/rhythm/docs/rd/modules/sleep-records/sleep-records.ui-ux.md) 中定义的 `record_result_summary_card`、`source_trust_badge_row`、`manual_record_form_sheet`、`late_reason_chip_group` 组件边界。
- 记录结果、来源可信度和当前可执行动作必须保持同一阅读序列，不允许在实现里拆成后台式列表或深层设置串联。
- `health` 数据读取、Drift 持久化和来源可信度换算只允许在数据边界处理；显示层只消费标准化记录、来源标签和可恢复错误状态。
