...
/// 验证首次引导会按欢迎、目标作息、增强设置顺序推进。
void main() {
  testWidgets('欢迎页继续后直接进入目标作息设置页，不再经过独立登录步骤', (tester) async {
    ...
    expect(find.text('STEP 1 · WELCOME'), findsNothing);
    ...
    expect(find.text('让系统帮你记录昨晚的睡眠'), findsNothing);
  });

  testWidgets('完成目标作息后进入增强设置页，完成后直接进入今日页', (tester) async {
    await pumpRhythmApp(tester, onboardingCompleted: false);
    await tester.pumpAndSettle();
  ...
  testWidgets('英文环境下首次引导不会混入中文文案', (tester) async {
    ...
    expect(find.text('Let Rhythm log last night for you'), findsNothing);
  });

  testWidgets('暗色主题下欢迎页使用主题化步骤卡底色', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(body: OnboardingWelcomeStep(onContinue: () {})),
      ),
    );
    await tester.pumpAndSettle();

    final checklistCard = tester.widget<Container>(
      find.byKey(const Key('onboarding-welcome-checklist-card')),
    );
    final checklistDecoration = checklistCard.decoration! as BoxDecoration;
    expect(
      checklistDecoration.color,
      AppTheme.dark().colorScheme.surface.withValues(alpha: 0.9),
    );

    final firstStepCard = tester.widget<Container>(
      find.byKey(const Key('onboarding-welcome-step-card-1')),
    );
    final firstStepDecoration = firstStepCard.decoration! as BoxDecoration;
    expect(
      firstStepDecoration.color,
